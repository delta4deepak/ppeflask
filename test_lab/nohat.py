...
def get_hat_boxes(detections):
        mask_list = detections.class_id
        indices = [i for i in range(len(mask_list)) if mask_list[i] == object]
        if not indices:
            return [], []
...
def no_hat_person(no_hat,result,detections):
        if result.boxes.id is not None:    
            if 5 in detections.class_id and object in detections.class_id and detections.xyxy.any():
                pb,tids = get_person_boxes(detections)
                mb = get_hat_boxes(detections)
                for pbox,tid in zip(pb,tids):
                    # print(tid)
                    for mbox in mb:
                        if inBoundingBox(pbox[0],pbox[1],pbox[2],pbox[3],center(mbox)[0],center(mbox)[1]):
                            if tid not in no_hat: no_hat.append(tid)
        return no_hat
